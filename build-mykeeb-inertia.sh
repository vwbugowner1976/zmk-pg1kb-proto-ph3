...
# cormoran's v0.3 custom Studio protocol is required by the already-working
# runtime input processor / Custom Settings setup.
[[ -f "$ZMK_APP/include/zmk/studio/custom.h" ]] || fail \
  "Current ZMK does not contain custom Studio RPC (app/include/zmk/studio/custom.h missing). Use the existing My Keeb Studio ZMK v0.3 custom-protocol checkout first."
...
CUSTOM_SETTINGS_DIR="$(module_path zmk-feature-custom-settings \
    "$ENV_ROOT/projects/zmk-feature-custom-settings" \
    "$ENV_ROOT/projects/zmk-feature-custom-settings-v03")" || fail "Custom Settings module not found"
